...
% Variants additions (defaults make it same as wce)

p.addParamValue('usetemp', true, @(x) isscalar(x) && islogical(x));
p.addParamValue('usenem',  false, @(x) isscalar(x) && islogical(x));
p.addParamValue('morttype', 'quadratic', @(x) ischar(x) && ismember(x, {'linear','quadratic','mixed','nekplankmix'}));
...
Biovars.usetemp  = In.usetemp;
Biovars.usenem   = In.usenem;
Biovars.morttype = In.morttype;
...
[newbio, db, WceOut, Diag, badthings] = integratebio(@wcevariantsode, t, dt, oldbio, Param, Biovars.odesolver{:});
