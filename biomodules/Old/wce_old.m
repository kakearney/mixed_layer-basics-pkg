...
% cfile = regexprep(In.outputfile, '.nc', '_total.dat');
% Biovars.cfid = fopen(cfile, 'wt');
% dz = -diff(Grd.zp(1:2));
% tot = sum(bio .* dz, 1); 
% Biovars.ntot0 = sum(tot(1:27));          % mol m^-2
...
[newbio, db, WceOut, Diag, badthings] = integratebio(@wceode, t, dt, oldbio, Param, Biovars.odesolver{:});
