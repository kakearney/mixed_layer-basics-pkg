function [newU newV] = solve_velocities(oldvels, mixcoef, dt, dz, cor, pgx, pgy, varargin)
...
if ~isnan(A.sflux_u)
    a(1) = 0;
    b(1) = 1 + thetad * kj(2) * (dt./dz(1)^2) + theta * A.dissipate * dt;
    c(1) = -thetad * kj(2) * (dt./dz(1).^2);
    e(1) = velj(1) + (1-thetad) * (-kj(2)*velj(1) + kj(2)*velj(2)) * ... 
           (dt./dz(1)^2) - (1-theta) * A.dissipate * velj(1) * dt + ...
...
if ~isnan(A.sflux_v)
    a(nz+1) = 0;
    b(nz+1) = 1 + thetad * kj(2) * (dt./dz(1)^2) + theta * A.dissipate * dt;
    c(nz+1) = -thetad * kj(2) * (dt./dz(1)^2);
    e(nz+1) = velj(nz+1) + (1-thetad) * (-kj(nz+2)*velj(nz+1) + ...
              kj(nz+2)*velj(nz+2)) * (dt./dz(1).^2) - (1-theta) * ...
              A.dissipate * velj(nz+1) * dt + A.source(nz+1) * dt - ...
              (1-theta) * cor * velj(1) * dt - pgy * dt + A.sflux_v * dt;
end

if ~isnan(A.bflux_u)
    a(nz) = -thetad * kj(nz) * (dt./dz(end).^2);
    b(nz) = 1 + thetad * kj(nz) * (dt./dz(end).^2) + theta * A.dissipate * dt;
    c(nz) = 0;
    e(nz) = velj(nz) + (1-thetad) * (-kj(nz)*velj(nz) + ...
            kj(nz)*velj(nz-1)) * (dt./dz(end)^2) - (1-theta) * ...
            A.dissipate * velj(nz) * dt + A.source(nz) * dt + ...
            (1-theta) * cor * velj(end) * dt - pgx * dt - A.bflux_u * dt;
end

if ~isnan(A.bflux_v)       
    a(end) = -thetad * kj(nz) * (dt./dz(end)^2);
    b(end) = 1 + thetad * kj(nz) * (dt./dz(end)^2) + theta * A.dissipate * dt;
    c(end) = 0;
    e(end) = velj(end) + (1-thetad) * (-kj(nz)*velj(end) + ...
             kj(nz)*velj(end-1)) * (dt./dz(end)^2) - (1-theta) * ...
