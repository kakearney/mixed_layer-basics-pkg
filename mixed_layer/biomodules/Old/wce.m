...
%WCE Water column ecosystem model
%
% [bioinit, ismixed, bottomval, Biovars, names] = wce('init', ...
%                                                              In, Grd);
% [newbio, diag] = wce('sourcesink', oldbio, meanqi, temp, z, dz, ...
%                    Biovars, t, dt);
% wsink = wce('vertmove', oldbio, meanqi, temp, z, dz, ...
%                    Biovars, t, dt);
%
% This module adapts the Ecopath with Ecosim (EwE) model for a
% one-dimensional water column.  The primary production included in the
% food web responds to specific environmental factors.  It also breaks the
% functional groups into planktonic and nektonic groups, where planktonic
% groups are confined to a specific layer of the water column model, while
% nektonic groups can freely move (and feed) throughout the water column
% (for computational purposes this also includes non-water-dwelling
...
p.addParamValue('diagnostic', {}, @(x) ischar(x) || (iscell(x) && all(cellfun(@ischar, x))));
...
canbeempty = {'diagnostic'};
default = setdiff(p.UsingDefaults, canbeempty);

if ~isempty(default)
    missing = sprintf('%s, ', default{:});
    error('Missing input parameter for wce biology: %s', missing(1:end-2));
end
...
% Ingestion flux forcing

if isfield(In, 'forceIngest')
    Biovars.forceIngest = cell(size(In.forceIngest));
    Biovars.forceIngest(:,1:2) = In.forceIngest(:,1:2);
    for ifc = 1:size(In.forceIngest,1)
        Biovars.forceIngest{ifc,3} = interptimedepth(In.forceIngest{ifc,3}, Grd);
    end
    Biovars.time = Grd.time;
else
    Biovars.forceIngest = [];
end
...
diagtokeep = {'dbdt', 'ps', 'growth', 'egest', 'excrete', 'natloss', 'remin'};
