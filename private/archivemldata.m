...
if Arch.islast(it)
    ...
    start = cell(size(avg));
%     [start{isd | isde}] = deal([0 Arch.bin(it)-1 Arch.iens-1]);
%     [start{iss}] = deal([Arch.bin(it)-1 Arch.iens-1]);
    [start{isd | isde}] = deal([0 Arch.bin(it)-1]);
    [start{iss}] = deal([Arch.bin(it)-1]);
    
    count = cell(size(avg));
%     [count{isd}] = deal([length(Grd.z) 1 1]);
%     [count{isde}] = deal([length(Grd.zp) 1 1]);
%     [count{iss}] = deal([1 1]);
...
%         ncwrite(fullfile(Arch.outfile, [data{iv,2} '.nc']), data{iv,2}, avg{iv}, start{iv});
    end
